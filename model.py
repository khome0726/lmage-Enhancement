...
def DecomNet(input_im, layer_num, channel=32, kernel_size=3):
    input_max = tf.reduce_max(input_im, axis=3, keepdims=True)
    input_im = concat([input_max, input_im])
    print("input_im_shape",input_im.get_shape())
    with tf.variable_scope('DecomNet', reuse=tf.AUTO_REUSE):
        conv = tf.layers.conv2d(input_im, channel, 3 * 3, padding='same', activation=None, name="shallow_feature_extraction")
        for idx in range(layer_num):
            conv = tf.layers.conv2d(conv, channel, kernel_size, padding='same', activation=tf.nn.relu, name='activated_layer_%d' % idx)
        conv = tf.layers.conv2d(conv, 2, kernel_size, padding='same', activation=None, name='recon_layer')

    R = tf.sigmoid(conv[:,:,:,0:1])
    L = tf.sigmoid(conv[:,:,:,1:2])

    return R, L

def RelightNet(input_L, input_R, channel=32, kernel_size=3):
    input_im = concat([input_R, input_L])
    print(input_im.get_shape())
    ...
    with tf.variable_scope('RelightNet'):
        ...
        print("up1 shape:", up1.get_shape().as_list())
        ...
        print("up2 shape:", up2.get_shape().as_list())
        ...
        print("up3 shape:", up3.get_shape().as_list())
        ...
        print("deconv1_resize shape:", deconv1_resize.get_shape().as_list())
        deconv2_resize = tf.image.resize_nearest_neighbor(deconv2, (tf.shape(deconv3)[1], tf.shape(deconv3)[2]))
        print("deconv2_resize shape:", deconv2_resize.get_shape().as_list())
        feature_gather = concat([deconv1_resize, deconv2_resize, deconv3])
        print("feature_gather shape:", feature_gather.get_shape().as_list())
        feature_fusion = tf.layers.conv2d(feature_gather, channel, 1, padding='same', activation=None)
        print("feature_fusion shape:", feature_fusion.get_shape().as_list())
...
class lowlight_enhance(object):
    def __init__(self, sess):
        ...
        print("I_delta : ", I_delta.get_shape())
        #I_low_3 = concat([I_low, I_low, I_low])
        #I_high_3 = concat([I_high, I_high, I_high])
        # I_delta_3 = concat([I_delta, I_delta, I_delta])
        # print("I_delta_3 : ", I_delta_3.get_shape())
        #tf.reshape(I_delta_3, [None, None, None, 1])

        self.output_R_low = R_low
        self.output_I_low = I_low
        self.output_I_delta = I_delta
        self.output_S = R_low * I_delta

        #self.output_S = tf.reshape(self.output_S, [None, None, None, 1])

        # loss
        self.recon_loss_low = tf.reduce_mean(tf.abs(R_low * I_low -  self.input_low))
        self.recon_loss_high = tf.reduce_mean(tf.abs(R_high * I_high - self.input_high))
        self.recon_loss_mutal_low = tf.reduce_mean(tf.abs(R_high * I_low - self.input_low))
        self.recon_loss_mutal_high = tf.reduce_mean(tf.abs(R_low * I_high - self.input_high))
        self.equal_R_loss = tf.reduce_mean(tf.abs(R_low - R_high))
        self.relight_loss = tf.reduce_mean(tf.abs(R_low * I_delta - self.input_high))
    ...
    def evaluate(self, epoch_num, eval_low_data, sample_dir, train_phase):
        ...
        for idx in range(len(eval_low_data)):
            ...
            input_low_eval = np.expand_dims(input_low_eval,axis=-1)
            print(input_low_eval.shape)
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
                ...
                for patch_id in range(batch_size):
                    h, w = train_low_data[image_id].shape[0], train_low_data[image_id].shape[1]
                    _ = 1 
                    x = random.randint(0, h - patch_size)
                    y = random.randint(0, w - patch_size)
            
                    rand_mode = random.randint(0, 7)
                    batch_input_low[patch_id, :, :, 0] = data_augmentation(train_low_data[image_id][x : x+patch_size, y : y+patch_size], rand_mode)
                    batch_input_high[patch_id,:, :, 0] = data_augmentation(train_high_data[image_id][x : x+patch_size, y : y+patch_size], rand_mode)
                ...
                # train
                _, loss = self.sess.run([train_op, train_loss], feed_dict={self.input_low: batch_input_low, self.input_high: batch_input_high, self.lr: lr[epoch]})
    ...
    def test(self, test_low_data, test_high_data, test_low_data_names, save_dir, decom_flag):
        ...
        for idx in range(len(test_low_data)):
            ...
            input_low_test = np.expand_dims(input_low_test, axis=-1)
